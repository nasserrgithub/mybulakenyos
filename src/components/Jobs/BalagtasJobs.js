...
class BalagtasJobs extends Component {
	constructor(props) {
		super(props)
		this.state = {
			data: [],
			count: '',
			buttontext: 'View Available Jobs in Balagtas, Bulacan '
		}
	}
	...
	fetchTable = () => {
		fetch('https://afternoon-garden-57638.herokuapp.com/jobsbalagtas')
		.then(response => response.json())
		.then(data => this.setState({data: data}))
	}

	fetchCount = () => {
		fetch('https://afternoon-garden-57638.herokuapp.com/jobsbalagtascount')
		.then(response => response.json())
		.then(data => this.setState({count: data.count}))
	}

	changeButtonText = () => {
		this.setState({buttontext: 'Jobs in Balagtas, Bulacan '})
	}
